...
TH1F *hEx = new TH1F("hEx","Background Subtraction on the ^{90}Zr Excitation Energy Spectrum",3100,-1.,30.); //previously the heading was "Excitation Energy"
TH1F *hExback1 = new TH1F("hExback1","Background Subtraction on the ^{90}Zr Excitation Energy Spectrum",3100,-1.,30.);
TH1F *hExback2 = new TH1F("hExback2","Background Subtraction on the ^{90}Zr Excitation Energy Spectrum",3100,-1.,30.);
TH1F *hExbacktot = new TH1F("hExbacktot","Background Subtraction on the ^{90}Zr Excitation Energy Spectrum",3100,-1.,30.);
TH1F *hExdiff = new TH1F("hExdiff","Background Subtraction on the ^{90}Zr Excitation Energy Spectrum",3100,-1.,30.);


//chain-150SmGroup
//Note that we use X1posC

DATA->Draw("31.3106-0.0316805*X1posC-3.5533e-07*X1posC*X1posC>>hEx",CUTbasic && CUTflags && CUTlimits && CUTy,"");
DATA->Draw("31.3106-0.0316805*X1posC-3.5533e-07*X1posC*X1posC>>hExback2",CUTbasic && CUTflags && CUTlimits && CUTyback2,"");
DATA->Draw("31.3106-0.0316805*X1posC-3.5533e-07*X1posC*X1posC>>hExback1",CUTbasic && CUTflags && CUTlimits && CUTyback1,"");
...
hExdiff->SetTitle("^{90}Zr(#alpha,#alpha') at 0^{o} (background subtracted)"); //figure out how to insert alpha,alpha'
hExdiff->SetStats(0);
...
hExdiff->Add(hEx);
hExdiff->Add(hExback1,-1.92);
hExdiff->Add(hExback2,-1.92);
hExbacktot->Add(hExback1,1.92);
hExbacktot->Add(hExback2,1.92);
